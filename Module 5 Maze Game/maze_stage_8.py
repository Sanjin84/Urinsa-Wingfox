#1. Create an enemy entity that shows up on level 3 (it will be a minotaur)
#2. Have the minotaur move in a basic manner
...
def load_level(lvl):
    ...
    if lvl == 3:
        minotaur = Entity(model ='cube', color=color.red,scale = (2,3,2), position = random.choice(empty_spaces))

def minotaur_movement(bull):
    project_forward = raycast(bull.position, bull.forward, distance = 1.05, ignore=[bull], debug= True)
